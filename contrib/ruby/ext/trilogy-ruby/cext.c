...
static void compact_trilogy(void *ptr)
{
    struct trilogy_ctx *ctx = (struct trilogy_ctx *)ptr;

    ctx->encoding = rb_gc_location(ctx->encoding);
    if (ctx->conn.socket) {
        ctx->conn.socket->opts.priv_data = (void *)rb_gc_location((VALUE)ctx->conn.socket->opts.priv_data);
    }
}
#else
...
static void mark_trilogy(void *ptr)
{
    ...
    if (ctx->conn.socket) {
        rb_gc_mark_movable((VALUE)ctx->conn.socket->opts.priv_data);
    }
}
...
static int _cb_ruby_connect(trilogy_sock_t *_sock)
{
    // noop: we dit it all in Ruby-land
    return TRILOGY_OK;
}

static int try_connect(struct trilogy_ctx *ctx, int fd, trilogy_handshake_t *handshake, const trilogy_sockopt_t *opts)
{
    trilogy_sock_t *sock = trilogy_sock_new_fd(opts, fd);
    if (sock == NULL) {
        return TRILOGY_ERR;
    }
    ...
    // replace the default connect implementationt o be a noop, because we did
    // the connection in Ruby-land.
    sock->connect_cb = _cb_ruby_connect;

    int rc = trilogy_connect_send_socket(&ctx->conn, sock);
    if (rc < 0) {
...
static VALUE rb_trilogy_connect(VALUE self, VALUE fd, VALUE encoding, VALUE charset, VALUE opts)
{
    struct trilogy_ctx *ctx = get_ctx(self);
    trilogy_sockopt_t connopt = {0};
    connopt.priv_data = (void *)self;
    ...
    int rc = try_connect(ctx, NUM2INT(fd), &handshake, &connopt);
    if (rc != TRILOGY_OK) {
        if (connopt.path) {
